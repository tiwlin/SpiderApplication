...
namespace CommonHelper
{
	...
	{
		#region 静态方法
		public static FtpClient CreateFTPClientByConfig()
		{
			string server = ConfigHelper.GetConfigString("FtpServer");
			string username = ConfigHelper.GetConfigString("FtpUser");
			string password = ConfigHelper.GetConfigString("FtpPassword");
			FtpClient clinet = new FtpClient(server, username, password);
			return clinet;
		}

		public static bool UploadFile(string filePath, string ftpFilePath)
		{
			bool rv = false;
			try
			{
				using(FtpClient client = FtpClient.CreateFTPClientByConfig())
				{
					if ((ftpFilePath ?? string.Empty).Length > 0)
					{
						client.ChangeDir(ftpFilePath);
					}
		...
		public Encoding DataEncoding = Encoding.Default;
			...
			try
			{
				...
				using(MemoryStream ms = new MemoryStream(1048576))
				{
					...
					while( timeout > DateTime.Now )
					{
						...
						//this.message += this.DataEncoding.GetString(buffer, 0, bytes);
						if ( bytes < this.buffer.Length ) break;
					}
					ms.Position = 0;
					using(StreamReader sr = new StreamReader(ms, this.DataEncoding))
					{
						this.message = sr.ReadToEnd();
					}
				}
				msg = Regex.Split(this.message.Trim() , @"\r?\n");
			}
			catch
			{
		...
		public void RenameFile(string oldFileName,string newFileName, bool overwrite)
		{
			...
			//this.sendCommand( "RENAME " + oldFileName + " " + newFileName);
		...
		private void sendCommand(String command)
		{
			...
			Byte[] cmdBytes = this.DataEncoding.GetBytes( ( command + "\r\n" ).ToCharArray() );
